...
static double DELTA_ANGLE=2*M_PI/360.0;
      ...
      requestNewMap(true)
{
    ...
    connect(dbThread,SIGNAL(HandleMapRenderingResult()), this,SLOT(redraw()));
...
void MapWidget::setAngle(double nangle) {
    angle=nangle/180*M_PI;
...
void MapWidget::HandleMouseMove(QMouseEvent* event)
{
    ...
    if (!projection.Move(startX-event->x(), event->y()-startY))
        return;
...
void MapWidget::paint(QPainter *painter)
{
    ...
    if (!dbThread->RenderMap(*painter,request) && requestNewMap) {
        TriggerMapRendering();
    }
...
{
    if (magnification.GetMagnification()/zoomFactor<min_magnification) {
        magnification.SetMagnification(min_magnification);
    }
    else {
        magnification.SetMagnification(magnification.GetMagnification()/zoomFactor);
    }
